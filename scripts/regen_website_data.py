...
def build_mini_products():
    ...
    for code, desc, section in catalogue_rows:
        ...
        next_id += 1
    print(f"  OK Mini products: {len(out)}  ({poa_count} POA in master, {out_count} not in master - both shown as POA)")
    return out
...
def write_featured_data_file(entries):
    if entries is None:
        ...
        print("  No featured-work.json - lib/featured-data.ts left as-is")
        return
    generated = "sync-data"
...
"""
    ...
    print(f"  OK Wrote lib/featured-data.ts ({len(entries)} entries)")
...
"""
    ...
    out_path.write_text(content, encoding="utf-8")
    print(f"  OK Wrote lib/mini-data.ts ({len(products)} products)")
...
"""
    ...
    out_path.write_text(content, encoding="utf-8")
    print(f"  OK Wrote lib/metals-data.ts ({len(metals)} metals)")
