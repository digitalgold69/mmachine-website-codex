...
assert.match(ordersClient, /invoice-print-payment[\s\S]+space-y-1 text-left[\s\S]+<strong>Cash:<\/strong>[\s\S]+<strong>Card:<\/strong>[\s\S]+<strong>BACS:<\/strong>/, "printed invoice payment methods must be stacked left in cash, card, BACS order");
assert.match(globalsCss, /body\.printing-invoice \.invoice-print-payment > div[\s\S]*text-align: left !important;/, "printed invoice payment methods must remain left aligned");
...
assert.match(ordersClient, /expectedUpdatedAt: quote\.updatedAt/, "dashboard saves must include the currently opened order version");
...
assert.match(ordersClient, /ADD_LINE_RESULT_BATCH = 30/, "add-line catalogue search must load a practical first batch");
assert.match(ordersClient, /loadMoreAddLineResults/, "add-line catalogue search must let staff load further rows for a scrollable picker");
assert.match(ordersClient, /Load next[\s\S]+ADD_LINE_RESULT_BATCH/, "add-line catalogue search must expose a clear load-more button");
...
assert.match(ordersClient, /lg:col-span-4[\s\S]+renderMetalLineMeasurementEditor\(item, index\)/, "metal measurement controls must expand across the full invoice row");
